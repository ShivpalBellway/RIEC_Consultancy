...
<div class="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
    ...
    <div class="px-6 py-5 border-b border-gray-100">
        <div class="flex items-center justify-between">
            <div>
                <h2 class="text-lg font-bold text-gray-900">All Success Stories</h2>
                <p class="text-sm text-gray-500 mt-1">Add, update, or remove testimonials and success stories.</p>
            </div>
            @if($items->total() > 0)
            <span class="text-sm text-gray-500">
                Showing {{ $items->firstItem() }} to {{ $items->lastItem() }} of {{ $items->total() }} stories
            </span>
            @endif
        </div>
    </div>
        ...
        <table class="w-full text-sm">
            <thead class="bg-gray-50 border-b border-gray-100">
                <tr>
                    ...
                    <th class="px-6 py-4 text-left font-bold text-gray-600">Name</th>
                    <th class="px-6 py-4 text-left font-bold text-gray-600">Role</th>
                    <th class="px-6 py-4 text-left font-bold text-gray-600">Review</th>
            ...
            <tbody class="divide-y divide-gray-100">
                @forelse($items as $index => $story)
                    <tr class="hover:bg-gray-50 transition" id="story-row-{{ $story->id }}">
                        ...
                        <td class="px-6 py-4">
                            @if($story->image)
                                <img src="{{ Str::startsWith($story->image, 'http') ? $story->image : asset('storage/' . $story->image) }}" alt="{{ $story->name }}"
                                     class="h-12 w-12 object-cover rounded-full border border-gray-200">
                            @else
                        ...
                        <td class="px-6 py-4">
                            ...
                            <label class="relative inline-flex items-center cursor-pointer">
                                <input type="checkbox"
                                       ...
                                       data-id="{{ $story->id }}"
                                       data-url="{{ route('admin.success-stories.toggle-status', $story->id) }}"
                        ...
                        <td class="px-6 py-4">
                            <div class="flex justify-end gap-2">
                                ...
                                <form action="{{ route('admin.success-stories.destroy', $story) }}"
                                      method="POST"
                    ...
                    <tr>
                        <td colspan="7" class="px-6 py-12 text-center text-gray-500">
                            <div class="flex flex-col items-center gap-2">
                                <i class="fa-solid fa-quote-right text-4xl text-gray-300"></i>
                                <p>No success stories added yet.</p>
                                <a href="{{ route('admin.success-stories.create') }}" class="text-blue-600 hover:text-blue-800 font-semibold">
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        if (window.adminToast) {
                            window.adminToast('success', `Story ${status ? 'activated' : 'deactivated'} successfully`);
                        }
                    } else {
