...
@section('title', 'Add Eligibility Field')
@section('page-title', 'Add Eligibility Field')
...
<form action="{{ route('admin.programs.eligibility.store', $program) }}" method="POST">
@csrf
...
<div class="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
    ...
    <div class="px-6 py-5 border-b border-gray-100 bg-gradient-to-r from-[#1a2f5e] to-[#0f1f45]">
        <div class="flex items-center justify-between gap-4">
            <div>
                <h2 class="font-extrabold text-white text-lg">New Eligibility Field</h2>
                <p class="text-xs text-white/70 mt-1">
                    For:
                    <span class="font-semibold text-[#dca737]">
    ...
    <div class="p-6 space-y-7 bg-gray-50/60">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
                ...
                <div class="xl:col-span-2">
                    ...
                    <input type="text" name="label" value="{{ old('label') }}" required
                           placeholder="e.g. Minimum Age, SEE GPA, IELTS Score"
                           class="form-input">
                    <p class="text-xs text-gray-400 mt-1">This label will appear on eligibility form</p>
                </div>
                ...
                <div>
                    ...
                    <select name="field_type" id="field_type" required onchange="toggleFieldOptions(this.value)"
                        ...
                        @foreach($fieldTypes as $key => $label)
                            <option value="{{ $key }}" {{ old('field_type', 'number') == $key ? 'selected' : '' }}>
                                {{ $label }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div id="unit-section">
                    ...
                    <input type="text" name="unit" value="{{ old('unit') }}"
                           placeholder="e.g. years, GPA, Band Score"
                           class="form-input">
                </div>
        ...
        <div class="bg-white rounded-2xl border border-gray-100 p-5">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
                ...
                {{-- Min Max --}}
                <div id="min-max-section" class="grid grid-cols-1 sm:grid-cols-2 gap-4 xl:col-span-1">
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-1.5">Min Value</label>
                        <input type="text" name="min_value" value="{{ old('min_value') }}"
                               placeholder="e.g. 17"
                               class="form-input">
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="max_value" value="{{ old('max_value') }}"
                               placeholder="e.g. 35"
                               class="form-input">
                    </div>
                </div>
                ...
                <div class="xl:col-span-3">
                    ...
                    <input type="text" name="validation_message" value="{{ old('validation_message') }}"
                           placeholder="e.g. IELTS score must be between 4.0 and 9.0"
                           class="form-input">
                    <p class="text-xs text-gray-400 mt-1">Shown to student when validation fails</p>
                </div>
            </div>
        </div>
        ...
        <div id="options-section" class="hidden bg-white rounded-2xl border border-gray-100 p-5">
            ...
            <div id="options-list" class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
                ...
                @else
                    <div class="flex gap-2 option-row">
                        <input type="text" name="options[]" class="form-input" placeholder="e.g. Level 1">
                        <button type="button" onclick="removeOption(this)"
                                class="w-11 h-11 shrink-0 rounded-xl bg-red-50 text-red-400 hover:bg-red-100 flex items-center justify-center text-xs transition">
                            <i class="fa-solid fa-times"></i>
                        </button>
