...
<div class="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
    ...
    <div class="px-6 py-5 border-b border-gray-100">
        <div class="flex items-center justify-between">
            <div>
                <h2 class="text-lg font-bold text-gray-900">All Features</h2>
                <p class="text-sm text-gray-500 mt-1">Add, update, or remove feature cards displayed on the homepage.</p>
            </div>
            @if($items->total() > 0)
            <span class="text-sm text-gray-500">
                Showing {{ $items->firstItem() }} to {{ $items->lastItem() }} of {{ $items->total() }} features
            </span>
            @endif
        </div>
    </div>
        ...
        <table class="w-full text-sm">
            <thead class="bg-gray-50 border-b border-gray-100">
                <tr>
                    ...
                    <th class="px-6 py-4 text-left font-bold text-gray-600">Subtitle</th>
            ...
            <tbody class="divide-y divide-gray-100">
                @forelse($items as $index => $feature)
                    <tr class="hover:bg-gray-50 transition" id="feature-row-{{ $feature->id }}">
                        ...
                        <td class="px-6 py-4">
                            @if($feature->icon)
                                <div class="w-10 h-10 rounded-xl bg-[#1a2f5e]/10 flex items-center justify-center text-[#1a2f5e]">
                        ...
                        <td class="px-6 py-4">
                            ...
                            <label class="relative inline-flex items-center cursor-pointer">
                                <input type="checkbox"
                                       ...
                                       data-id="{{ $feature->id }}"
                                       data-url="{{ route('admin.features.toggle-status', $feature->id) }}"
                    ...
                    <tr>
                        <td colspan="6" class="px-6 py-12 text-center text-gray-500">
                            <div class="flex flex-col items-center gap-2">
                                ...
                                <a href="{{ route('admin.features.create') }}" class="text-blue-600 hover:text-blue-800 font-semibold">
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        if (window.adminToast) {
                            window.adminToast('success', `Feature ${status ? 'activated' : 'deactivated'} successfully`);
                        }
                    } else {
