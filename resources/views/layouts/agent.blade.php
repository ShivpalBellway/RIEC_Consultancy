...
<head>
    ...
    <style>
        ...
        .sidebar-item.active { background: #dca737; color: #ffffff; font-weight: 600; box-shadow: 0 4px 12px rgba(220, 167, 55, 0.3); }
        .sidebar-item:not(.active):hover { background: rgba(255, 255, 255, 0.1); color: #ffffff; }
    </style>
    @stack('styles')
</head>
<body class="bg-gray-50 font-sans text-gray-800 antialiased min-h-screen flex flex-col">
    ...
    <div x-data="{ sidebarOpen: false }" class="flex h-screen overflow-hidden">
        ...
        <!-- Sidebar -->
        <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'" class="fixed inset-y-0 left-0 z-40 w-64 bg-primary-600 text-white transition-transform duration-300 ease-in-out md:static md:translate-x-0 flex flex-col justify-between shadow-xl">
            <div>
                <!-- Brand Header -->
                <div class="h-16 flex items-center justify-between px-6 border-b border-primary-500/30">
                    <a href="{{ route('agent.dashboard') }}" class="flex items-center gap-3">
                        <div class="w-9 h-9 rounded-lg bg-gold flex items-center justify-center text-white font-extrabold text-lg shadow-md">
                            R
                        </div>
                        <div>
                            <span class="font-extrabold text-lg tracking-tight text-white">REIAC</span>
                            <span class="text-xs text-gold block font-semibold -mt-1 tracking-wider uppercase">Agent Portal</span>
                        </div>
                    </a>
                ...
                <!-- Agency Info Badge -->
                <div class="px-5 py-4 m-3 rounded-xl bg-primary-700/60 border border-primary-500/30 backdrop-blur-sm">
                ...
                <!-- Nav Links -->
                <nav class="px-3 py-2 space-y-1.5">
